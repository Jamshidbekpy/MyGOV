...
<body>";
 ...
 $jamsumma_s=$jamsumm*0.12;
  ...
  $inforq="";
    $mus=0;  
$today = new DateTime(); // hozirgi sana
...
for ($i = $oy; $i > 0; $i--) {
    ...
    $yil = $date->format("Y");
    $oyNomi = $date->format("F"); // Oyning nomi inglizchada (xohlasa uzbekchaga o‘girsa bo‘ladi)

    // === 1-ISH ===
    if (!empty($row['ish_b']) && !empty($row['ish_t'])) {
        $ib = new DateTime($row['ish_b']);
        $it = ($row['ish_t'] === 'До сих пор') ? $today : new DateTime($row['ish_t']); // agar "До сих пор" bo‘lsa hozirgi sana
        ...
        if ($date >= $ib && $date <= $it) { 
            ...
            $inforq .= "<td>$yil</td>";
            $inforq .= "<td>$oyNomi</td>";
            $inforq .= "<td>{$row['ish_joyi']}</td>";
            $inforq .= "<td>{$summa}</td>";
            $inforq .= "<td>{$summa_so}</td>";
            $inforq .= "<td>0</td>";
            $inforq .= "</tr>";
        }
    }
    ...
    // === 2-ISH ===
    if (!empty($row['ish_b1']) && !empty($row['ish_t1'])) {
        $ib1 = new DateTime($row['ish_b1']);
        $it1 = ($row['ish_t1'] === 'До сих пор') ? $today : new DateTime($row['ish_t1']); // "До сих пор" bo‘lsa hozirgi sana

        if ($date >= $ib1 && $date <= $it1) {
            $mus += $summa1q;

            $inforq .= "<tr style='text-align:center'>";
            $inforq .= "<td>$yil</td>";
            $inforq .= "<td>$oyNomi</td>";
            $inforq .= "<td>{$row['ish_joyi1']}</td>";
            $inforq .= "<td>{$summa1}</td>";
            $inforq .= "<td>{$summa_so1}</td>";
            $inforq .= "<td>0</td>";
            $inforq .= "</tr>";
        }
    }
...
работы</th>
       ...
       $infor11.=" <tr><td style='text-align:center'>1</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['ish_b'] . "</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['ish_t'] . "</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['ish_joyi'] . "</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['stir'] . "</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['doljin'] . "</td>"; // Full month name in English
       $infor11.=" <td style='text-align:center'>" .$row['otdel'] . "</td>"; // Full month name in English
                 $infor11.="</tr>";
        ...
        $infor11.=" <tr><td style='text-align:center'>2</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['ish_b1'] . "</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['ish_t1'] . "</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['ish_joyi1'] . "</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['stir'] . "</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['doljin1'] . "</td>"; // Full month name in English
        $infor11.=" <td style='text-align:center'>" .$row['otdel1'] . "</td>"; // Full month name in English
                  $infor11.="</tr>";
...
$_REQUEST['data']="https://my.gov.uz/ru/repo/get-document/0a6d-cc15-4156-80ef-5105-f342-c17f";
// $_REQUEST['data']="http://localhost/mygov/file/download/$numbers11";
